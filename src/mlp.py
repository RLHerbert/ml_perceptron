...
class mlp:
    def __init__(self, n_hidden_nodes, n_outputs):  # , n_inputs):
        ...
        self.hidden_layer = [{'weights': [
            uniform(-1.0, 1.0) for i in range(n_inputs + 1)]} for i in range(n_hidden_nodes)]
        self.output_layer = [{'weights': [
            uniform(-1.0, 1.0) for i in range(n_hidden_nodes + 1)]} for i in range(n_outputs)]
    ...
    def train(self, dataset):
        ...
        for example in dataset:
            ...
            self._forward_prop(self.hidden_layer,
                               self.output_layer, attribute_vector)
            self._backprop(self.hidden_layer, self.output_layer, output_vector,
                           dataset[:][len(example)-1], hidden_vector, attribute_vector)
    ...
    def _backprop(self, hidden_layer, output_layer, output_vector, target_vector, hidden_vector, attribute_vector, eta=0.1):
        ...
        output_responsibility = np.outer(
            np.outer(output_vector, (1 - output_vector)), (target_vector - output_vector))
        hidden_responsibility = np.outer(np.outer(
            hidden_vector, (1 - hidden_vector)), output_responsibility.dot(output_layer))

        output_layer = output_layer + eta * \
            np.outer(np.array([output_responsibility, ] *
                              num_hid).transpose(), hidden_vector)
        hidden_layer = hidden_layer + eta * \
            np.outer(np.array([hidden_responsibility, ] *
                              num_out).transpose(), attribute_vector)
...
if __name__ == "__main__":
    ...
    forward_prop_results = MLP._forward_prop(
        hidden_layer, output_layer, attribute_vector)
    # Hidden node outputs
    ...
    backprop_results = MLP._backprop(
        hidden_layer, output_layer, output_vector, target_vector, hidden_vector, attribute_vector)
