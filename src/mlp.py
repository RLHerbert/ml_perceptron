...
class mlp:
    def __init__(self, n_hidden_nodes, n_outputs):
        ...
        self.hidden_layer_weight = [
            [uniform(-1.0, 1.0) for i in range(n_inputs)] for i in range(n_hidden_nodes)]
        self.output_layer_weight = [
            [uniform(-1.0, 1.0) for i in range(n_hidden_nodes)] for i in range(n_outputs)]
    ...
    def train(self, dataset):
        ...
        for example in dataset:
            ...
            print("elf.hidden_layer_weight ", len(
                self.hidden_layer_weight), len(self.hidden_layer_weight[0]))
            # print(self.hidden_layer_weight)

            print("self.output_layer_weight ", len(
                self.output_layer_weight), len(self.output_layer_weight[0]))
    ...
    def _backprop(self, hidden_layer_weight, output_layer_weight, output_layer_neurons, target_vector, hidden_layer_neurons, attribute_vector, eta=0.1):
        ...
        output_responsibility = np.multiply(np.multiply(
            output_layer_neurons, (1 - output_layer_neurons)), (target_vector - output_layer_neurons))
        hidden_responsibility = np.multiply(np.multiply(hidden_layer_neurons, (
            1 - hidden_layer_neurons)), output_responsibility.dot(output_layer_weight))
        ...
        hidden_layer_weight = hidden_layer_weight + eta * \
            np.multiply(np.array([hidden_responsibility, ]
